...
public class UsersActivity extends BaseTActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<UserIn> users_list=LitePal.findAll(UserIn.class);
        ...
        if ((newsList!= null&&!newsList.isEmpty())||(newsList2!=null&&!newsList2.isEmpty()) ){
            if (newsList!= null&&!newsList.isEmpty()) {
                for (int i = 0; i < newsList.size(); i++) {
                    ...
                    values.put("i", 0);
                    LitePal.updateAll(UserIn.class, values, "i = ?", "2");
                }
            }
            if (newsList2!=null&&!newsList2.isEmpty()){
                for (int i = 0; i < newsList2.size(); i++) {
                    ContentValues values = new ContentValues();
                    values.put("i", 1);
                    LitePal.updateAll(UserIn.class, values, "i = ?", "3");
                }
            }
        }
    }
