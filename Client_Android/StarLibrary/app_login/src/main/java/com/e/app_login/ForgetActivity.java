...
public class ForgetActivity extends BaseTActivity {
     ...
     * 返回登录
     * */
    public void OnBack(View view) { ForgetActivity.this.finish();}
    /*
     * 忘记密码
     * */
    public void forget(View view) {
        ...
        if (userPassword.length()<=16&&userPassword.length()>=6&&pattern.matcher(userPhone).matches()) {
            if (NetWork.isNetworkConnected(getApplicationContext())) {
                HttpUtil.sendOkHttpRequest("https://starlibrary.online/haopeng/user/forgetpassword" +
                        "?userName=" + userName + "&userPassword=" + userPassword + "&userPhone=" + userPhone, new Callback() {
                    @Override
                    ...
                    private void showResponseDate(final String responseData) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                JsonRootBean jsonRootBean = new Gson().fromJson(responseData, JsonRootBean.class);
