...
@Route(path = "/app_borrow/ReservingActivity")
public class ReservingActivity extends AppCompatActivity {
    ...
    private void collectiondetail(){
        if (NetWork.isNetworkConnected(this)){
            HttpUtil.sendOkHttpRequest("https://starlibrary.online/haopeng/bookBorrow/borrowdetail?card="+card+"&type=0", new Callback() {
                @Override
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (jsonMsgOrder.getStatus() == 200) {
                                LinearLayoutManager linearLayoutManager=new LinearLayoutManager(ReservingActivity.this);
                                RecyclerView rv =findViewById(R.id.reserving_recyclerview);
